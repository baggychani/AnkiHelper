...
function App() {
  ...
  const exportFile = async (kind: ExportKind, mediaType?: MediaKind) => {
    if (!selected) return
    const details = {
    ...
    const mediaSuffix = kind === 'media' && mediaType ? `_${mediaLabels[mediaType]}` : ''
    ...
    try {
      const response = await fetch(api.exportUrl(kind, selected.id, mediaType))
