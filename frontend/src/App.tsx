...
import { api, type MediaKind, type TablePreview, type Workspace } from './api'
...
function App() {
  ...
  const [clozeCards, setClozeCards] = useState<{ ordinals: number[]; active: number }>({ ordinals: [], active: 0 })
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    const timer = window.setTimeout(() => api.preview(selected.id, templateIndex, previewState.side, previewState.noteIndex, previewState.clozeOrdinal, controller.signal)
      .then(({ html, cloze_ordinals, cloze_ordinal }) => {
        if (controller.signal.aborted) return
        setPreviewDocument({ key: requestKey, html: `${html}<!-- anki-helper-preview:${++previewRevision.current} -->` })
        setClozeCards({ ordinals: cloze_ordinals ?? [], active: cloze_ordinal ?? 0 })
      })
      .catch((caught) => {
  ...
  const exportFile = async (kind: ExportKind, mediaType?: MediaKind, storedNames?: string[]) => {
    if (!selected) return
    const details = {
    ...
    const mediaLabels: Record<MediaKind, string> = { audio: '음성', image: '이미지', video: '영상', font: '폰트', other: '기타' }
    const mediaSuffix = kind === 'media' && mediaType ? `_${mediaLabels[mediaType]}` : kind === 'media' && storedNames?.length ? '_선택' : ''
    const destination = await save({ defaultPath: `${selected.name.replace(/[\\/:*?"<>|]/g, '_')}_${details[1]}${mediaSuffix}.${details[0]}`, filters: [{ name: details[2], extensions: [details[0]] }] })
    if (!destination) return
    try {
      const response = await fetch(api.exportUrl(kind, selected.id, mediaType, storedNames))
