...
- 노트 유형별 카드 데이터 확인
- 일반 데이터 셀 더블클릭 편집
- 필드 이름 변경·추가·삭제 및 별도 노트 유형으로 복제
...
일반 저장은 현재 열어 둔 APKG에 변경사항을 반영합니다. 저장 직전에 수정 전 APKG가 `AnkiHelper Backups` 폴더에 시간별로 백업됩니다. 원본과 별개의 파일이 필요하면 사이드바의 **다른 이름으로 APKG 저장**을 사용하세요.
...
완전히 새 APKG를 만들려면 노트와 필드 외에도 Anki 컬렉션 데이터베이스, 덱·노트 유형 ID, 카드 생성 규칙, GUID, 미디어 매핑 등을 새로 만들어야 합니다. 이 기능은 현재 1.0.0의 범위에 포함되지 않습니다.
...
AnkiConnect 실시간 연동은 아직 포함하지 않습니다. 현재 1.0.0은 APKG 파일 모드와 Anki Helper 편집 프로젝트의 안전한 왕복 편집에 집중합니다.
